...
int UWB_send_timer = 4000; // when to send. 
...
void loop() {
  ...
  if (!print_time && GPS.fix){
    ...
    if(!logFile.open(logFilename, O_APPEND | O_WRITE)) {
      Serial.println("Could not create log file.  Check SD Card. 0");
    }
    logFile.print("Time: ");
    if (GPS.month < 10)
      logFile.print("0");
...
void printGPSInfo(float lat, float lon, float speed, float altitude, int numSats) {
  ...
  noInterrupts();
  if(!logFile.open(logFilename, O_APPEND | O_WRITE)) {
    Serial.println("Could not create log file.  Check SD Card. 1");
  }
  logFile.print("GPS: ");
  logFile.print(millis());
  logFile.print(",");
...
void printIMUInfo(float xAccel, float yAccel, float zAccel, float xGyro, float yGyro, float zGyro, float xMag, float yMag, float zMag, float temp, bool logSD) {
  ...
  if (logSD) {
    ...
    noInterrupts();
    if(!logFile.open(logFilename, O_APPEND | O_WRITE)) {
        Serial.println("Could not create log file.  Check SD Card. 2");
    }
...
void newRange() {
  ...
  noInterrupts();
  if(!logFile.open(logFilename, O_APPEND | O_WRITE)) {
        Serial.println("Could not create log file.  Check SD Card. 3");
  }
  logFile.print("UWB: ");
...
void logUWBToUSB(){
  char buff[140];
  char long_buff[140]; // originally 1400
  ...
  noInterrupts();
  if(!logFile.open(logFilename, O_APPEND | O_WRITE)) {
    Serial.println("Could not create log file.  Check SD Card. 4");
  }
